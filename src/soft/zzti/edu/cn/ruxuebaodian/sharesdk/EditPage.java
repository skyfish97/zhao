...
package soft.zzti.edu.cn.ruxuebaodian.sharesdk;
...
public class EditPage extends FakeActivity implements OnClickListener, TextWatcher, Callback {
	...
	private void initPageView() {
		...
		LayoutParams lpInput = new LayoutParams(
				LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
		...
		if(!TextUtils.isEmpty(imagePath) && new File(imagePath).exists()){
			...
			llRight.setLayoutParams(new LayoutParams(
	...
	public void afterPlatformListGot() {
		...
		LayoutParams lpItem = new LayoutParams(dp_36, dp_36);
		...
		for (int i = 0; i < size; i++) {
			...
			if (i >= size - 1) {
				fl.setLayoutParams(new LayoutParams(dp_36, dp_36));
			}
			llPlat.addView(fl);
